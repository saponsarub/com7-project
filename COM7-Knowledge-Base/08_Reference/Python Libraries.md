...
| หลัก                                    | เหตุผล                                                                                                              |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| **stdlib **                             | `urllib` แทน `requests` ทำให้ zip เล็กลงและไม่มี dependency ให้ดูแล                                                 |
...
## แพ็กยังไง — Layer vs Function package

```
/var/task/     function package   โค้ด + ไฟล์ที่โค้ดเปิดด้วย open()
/opt/python/   layer              library ทั้งหมด
```

| เกณฑ์ | อยู่ที่ไหน |
|---|---|
| เปลี่ยนพร้อมโค้ด (โลโก้ · template) | **function package** |
| นาน ๆ เปลี่ยนที (library) | **layer** |

**ไฟล์ที่โค้ดเปิดด้วย `open()` ต้องอยู่ใน function package** — `sys.path` ใช้ตอน `import` เท่านั้น ไม่ได้ช่วยเรื่อง `open()`

**แยก layer แล้ว zip ของ function เหลือต่ำกว่า 3 MB จะแก้โค้ดใน Console ได้** ซึ่งช่วยมากตอนปรับหน้าตาอีเมล
