...
**CRM มี `identity_no` จริงหรือไม่** — แบบสำรวจตอบ `3 (ไม่มี)` แต่กลับเขียนชื่อฟิลด์ `identity_no` มาด้วย
เป็นแถวเดียวในไฟล์ที่ขัดกันเอง · ถ้ามีจริงจะเปลี่ยนภาพความพร้อมของ CRM ทั้งหมด → [[CRM Customer Field Survey]]

**`subdistrict` ใน CRM เก็บอำเภอหรือตำบล** — แบบสำรวจ map `อำเภอ/เขต` ไปที่ `subdistrict`
ซึ่งแปลว่าตำบล · ต้องเปิดดูค่าจริง ไม่ใช่เชื่อชื่อคอลัมน์

**ชื่อลูกค้าใน CRM ไม่แยกภาษา** — ผู้ตอบระบุว่า *"แล้วแต่ลูกค้ากรอกมา ไม่ได้ฟิล ภาษาไทย ภาษาอังกฤษ"*
คนเดียวกันอาจเป็น `สมชาย` ใน CRM แต่ `Somchai` ใน K2 → จับคู่ข้ามระบบพลาด

**ไม่มีระบบไหนเก็บ customer activity เลย** — banana app / banana web / studio7 web / shopee
ทั้ง 4 ช่องทางตอบว่าไม่มีเหมือนกันหมด ถ้าจะทำ personalization ต้องเริ่มจากศูนย์

---


## UFUND / Customer 360 — คำถามที่ยังไม่มีคำตอบ

> ยุบจาก `Customer Data Issues` (2026-09-16) · รายละเอียด → [[UFUND in Customer 360]]

- **นิยาม "ลูกค้า Active"** — สัญญาที่ยังผ่อน หรือคนที่เคยเป็นลูกค้า · ตัวเลข 200–300k ต่างจากที่นับใน K2 (103k) 2–3 เท่า
- ตัวเลข 500,000 ราย นับรวม prospect ที่ยังไม่ยื่นคำขอไหม
- ถ้า Student อยู่บน K2 ต่อ **จะรายงาน ธปท. จากอะไร** (K2 ไม่มีคอลัมน์รหัส ธปท.)
- Indicator ของ NCB จะคำนวณที่ไหน — ITOS ก่อนส่งออก หรือใน lake
- สัญญาที่ migrate K2→ITOS แล้ว **จะซ้ำกันไหม**
- K2 จะปิดจริงเมื่อไหร่ · ข้อมูลเก่า archive ที่ไหน
- ช่วงเปลี่ยนผ่าน ลูกค้าที่มีสัญญาทั้งสองระบบ dedupe ด้วยอะไร (`TAX_ID` เป็นตัวเดียวที่มีทั้งคู่)
...
[[SSOT Roadmap]] · [[Current Status]] · [[Decisions]] · [[Consent & PDPA]]
